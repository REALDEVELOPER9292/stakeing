...
contract Staking is Ownable, ReentrancyGuard {
    ...
    uint256 private _MIN_STAKE_AMOUNT = 100 * 10 ** _DECIMALS;
    uint256 private _MAX_STAKE_AMOUNT = 10000 * 10 ** _DECIMALS;
    uint256 private _REWARD_CYCLE = 5 minutes;
    uint256 private _REWARD_RATE = 50; // x0.5
    uint256 private _currentStakedAmount = 0;
    ...
    uint256 public timeflag = 0;

    constructor (address _stakingToken) {
        require(Address.isContract(_stakingToken), "The address does not point to a contract.");
    ...
    function claimReward(uint256 _id) external nonReentrant{
        ...
        timeflag = claimableCycle;
        uint256 rewardAmount = idToStake[_id].amount.mul(_REWARD_RATE).div(100).mul(claimableCycle);
